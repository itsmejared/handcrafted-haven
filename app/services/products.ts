...
export async function getProducts(params: ProductQueryParams) {
  const { category_id, product, seller_id, min_price, max_price, sort, page, limit } =
    params;
  ...
  try {
    ...
    // Filter by seller_id (used by "My Listings" to show only a seller's own products)
    if (seller_id !== undefined && seller_id.trim().length > 0) {
    ...
    // 5. Consulta de datos: incluye reviews (rating/count) y product_images
    // (todas las imágenes de cada producto, agregadas como JSON, para
    // vistas que necesiten mostrar una galería en lugar de una sola imagen).
    const dataQuery = `
