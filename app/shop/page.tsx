...
export default async function ShopPage({
  ...
  try {
    ...
    `;
    const groupBy = `GROUP BY p.id, u.name, c.name`;
 
    const result = category
      ? await db.query(
          `${baseQuery} WHERE c.id = $1 ${groupBy} ORDER BY p.created_at DESC`,
          [category],
        )
      : await db.query(`${baseQuery} ${groupBy} ORDER BY p.created_at DESC`);
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto">
            {products.map((product) => {
              const roundedRating = Math.round(Number(product.average_rating));
              return (
<div
                  key={product.id}
...
<span className="ml-2">{Number(product.average_rating).toFixed(1)}</span>
<span className="text-[#3D2B1F] opacity-60"> ({product.review_count} reviews)</span>
</div>
